...
public:
	...
	friend BOOL IsZero(const CVect3 &v, double eps);
	...
	// friend CVect3 pow(const CVect3 &v, int k=2);	
	...
	friend CVect3 pp2vn(const CVect3 &pos1, const CVect3 &pos0, double ts, CEarth *pEth); 
...
public:
	...
	friend double normInf(const CVect &v);	
	// friend CVect pow(const CVect &v, int k=2);	// inf-norm
...
public:
	...
	// friend void DVMDVafa(const CVect &V, CMat &M, double afa=1.0);
