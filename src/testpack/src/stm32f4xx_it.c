...
void TIM3_IRQHandler(void)
{
	if (TIM_GetITStatus(TIM3, TIM_IT_Update) != RESET) 
  {
		 ...
		 MS5611_cnt++;
		 if(MS5611_cnt==1)                      // 分5步读温度和气压，10Hz输出
			{
...
void USART2_IRQHandler(void)   // GPS输入中断
{
	char temp;
  ...
  {	
      ...
      USART_ClearITPendingBit(USART2, USART_IT_RXNE);//清楚中断标志
		  ...
		  if(PPs_cnt>0)
	    {
				...
				Length2++;
				if((Length2==1)&&(Rx2_data1[0]!=0xb5)) // 0xb562: u-center->View->Message View->UBX->NAV->PVT
				{
					Rx2_data1[0]=0;
					Length2=0;
				}
...
char ubloxcmd[32]; int ii=0;

void USART1_IRQHandler(void)    // COM1/USB 的输入中断
{
//	if(mcu_init_gpscfg)
	{
		char temp;
		...
		{	
				...
				ubloxcmd[ii++] = temp; if(ii>=32||temp==0) ii=0;
		}	
		return;
	}
	/*
